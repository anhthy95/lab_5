...
function loadStatsPage(){
	...
	for(row = 2; row < table.rows.length; row++){
		var homeScore = Number(table.rows[row].cells[2].innerHTML);
		
		var oppScore = Number(table.rows[row].cells[3].innerHTML);
...
function loadPlayersPage(){
	...
	for(var i = 0; i < players.length; i++){
		...
		if (i==0) tag.onclick = function(){switchPlayers(0);};
				else if(i==1) tag.onclick = function(){switchPlayers(1);};
				else if(i==2) tag.onclick = function(){switchPlayers(2);};
				else if(i==3) tag.onclick = function(){switchPlayers(3);};
		playerSelect.appendChild(tag);
	}
}				
